...
| WF-13 | `params.krona = true` renders one chart per occurrence table, each named after the table it was built from — `sintax.tsv` → `sintax.krona.html`, `sintax_optimistic.tsv` → `sintax_optimistic.krona.html` — published beside those tables in `outdir` (a); the default (`false`) runs no `KRONA` and produces no HTML (b); a non-boolean `krona` aborts at startup via the aggregated validation report (c); a run that sets `table_name` gets charts carrying that name (d), so parallel runs' charts stay distinguishable once they leave their `outdir` — the fixed `krona.html` of v1.13.0 and earlier collided in any directory two runs' charts were gathered into. |
...
| SX-17  | Every external tool the script invokes must be **usable**, not merely present: `check_commands` runs `cutadapt --version` and `vsearch --version` at startup, and a tool whose invocation fails aborts the script with a clear stderr error naming it and quoting its output. Presence is not usability — a pip/pipx console script whose interpreter or `site-packages` has gone away still satisfies `command -v`, so a broken cutadapt used to reach `trim_primers` and die there as a bare exit 1 (see SX-18). |
| SX-18  | A tool failure inside `trim_primers` is reported on **stderr**: the offending FASTQ file is named and cutadapt's output for that file is echoed, as well as appended to `<barcode>.log`. cutadapt's stderr goes to the log so the log stays a complete trimming report, but that alone left Nextflow an empty `.command.err` — and Nextflow omits its `Command error:` section entirely when stderr is empty, so the failure was undiagnosable from the run report. |
| SX-19  | The barcode's FASTQ files may be supplied through `--fastq-list FILE` (one path per line) as well as positionally; the two combine, blank lines are ignored, and every listed path is validated like a positional one (a missing or unreadable list file is itself an error). The `SINTAX` module passes the file set this way, which lifts the argv ceiling: ~32,000 names of 56 characters exhaust `ARG_MAX` (2 MiB under a default 8 MB stack) and the task dies with exit 126 and `Argument list too long`, whereas a heredoc in the task script is read from a file and has no such limit. |
...
| ID     | Specification                                                                                                                                          |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| KR-40  | `build_krona.sh` on an occurrence table produces a non-empty `<table stem>.krona.html` that carries a Krona marker and the fixture's leaf taxa.         |
...
| KR-42  | Given both tables, the driver names each chart after its own input table's stem: `occurrence.tsv` → `occurrence.krona.html`, `occurrence_optimistic.tsv` → `occurrence_optimistic.krona.html` (a). The stem is taken whole rather than keyed off an `_optimistic` substring, so a table that is *itself* named `*_optimistic.tsv` no longer sends both charts to one filename — which overwrote one of them and emitted a single HTML where the module declares two (b). Must agree with `krona_name()` (FN-11), which names the same files in the module's stub. |
...
| FN-11  | `krona_name(name)` is the Krona chart named after an occurrence table: the final extension is replaced by `.krona.html` — `sintax.tsv` → `sintax.krona.html` (a); `table.txt` → `table.krona.html` (b); an extensionless `table` → `table.krona.html` (c); it splits on the **last** dot only, so `run.1.tsv` → `run.1.krona.html` (d); a leading dot is not an extension, so `.hidden` → `.hidden.krona.html` (e) — the same rules `optimistic_name` follows (FN-03). Must agree with `html_name_for()` in `build_krona.sh` (KR-42): that names the files a real run publishes, this names the stub's. |
...
| PRM-03 | Every parameter `main.nf` reads is declared with a default in `nextflow.config`, so a default (non-cluster) run emits no `Access to undefined parameter` warning. `params.require_slurm_account` was read on every run but declared only in `conf/slurm.config`, which nothing but the cluster profiles loads — so every local run printed that warning above its summary, where it reads like a misconfiguration rather than the harmless default it is. An `?:` fallback does not help: Nextflow warns on the *read*, before the elvis operator sees a value. The mirror of PRM-01 — that one keeps the user from passing a name the pipeline does not declare, this one keeps the pipeline from reading one itself. |
